---
  title: "Estimation de la surface par modèle linéaire à coefficients variant spatialement - MONO-FACTEUR - Département du Rhône"
...
Initialisation de fonctions utiles pour le krigeage
...
load(file = "../Data/CCR_data_Rhone.Rdata")
area_nm <- "Rhône"
```

```{r fn_limites, echo=FALSE, eval=TRUE}
...
# Capping mono-variable par le quantile 98%
Q98_M_NB   <- quantile(DAT$N4NBPIEC[DAT$C1NATURE == 1], probs = 0.98, na.rm = TRUE)
Q98_A_NB   <- quantile(DAT$N4NBPIEC[DAT$C1NATURE == 2], probs = 0.98, na.rm = TRUE)
Q98_M_S    <- quantile(DAT$N4SURFAC[DAT$C1NATURE == 1], probs = 0.98, na.rm = TRUE)
Q98_A_S    <- quantile(DAT$N4SURFAC[DAT$C1NATURE == 2], probs = 0.98, na.rm = TRUE)
NB_cap     <- rep(NaN, nrow(DAT))
NB_cap[DAT$C1NATURE == 1] <- pmin(DAT$N4NBPIEC[DAT$C1NATURE == 1], Q98_M_NB)
NB_cap[DAT$C1NATURE == 2] <- pmin(DAT$N4NBPIEC[DAT$C1NATURE == 2], Q98_A_NB)
S_cap     <- rep(NaN, nrow(DAT))
S_cap[DAT$C1NATURE == 1] <- pmin(DAT$N4SURFAC[DAT$C1NATURE == 1], Q98_M_S)
S_cap[DAT$C1NATURE == 2] <- pmin(DAT$N4SURFAC[DAT$C1NATURE == 2], Q98_A_S)
DAT <- cbind(DAT, NB_cap, S_cap)

tab <- matrix(c(Q98_M_NB, Q98_M_S, Q98_A_NB, Q98_A_S), 2,2, byrow = TRUE)
colnames(tab) <- c(nm_X, nm_Y)
rownames(tab) <- c("Maisons", "Appartements")
knitr::kable(tab, caption = "Valeurs des seuils d'écrêtage (Quantile 98%)")
```
...
save(list=c("dat","dpts"),file="../Data/CCR_69.Rdata")
```
