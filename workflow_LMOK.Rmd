...
flag.new <- FALSE
if (!flag.new) {
  # initial version of the kfold function
  datXval <- SPDE_LMOK_kfold_compute(datXval, model = m_est, mesh = mesh, 
                                     radix = "kfold", verbose = TRUE)
} else {
  # new version of the kfold function
...
dat = prepareDb(dat, 
                ...
                type = type,
                sigma = resModel$sigma)
datXval <- db.add(dat, folds,loctype="code")

# definition de la fonction d'estimation à évaluer
fn_estim <- function(dbin, dbout, radix = "kfold_SPDE_LMOK"){
  SPDE_LMOK_krigsim(dbin, dbout,
>>>>>>> 2b61fa284483322a74ae5d33f531f0b9f693bb1e
                    model = m_est,
                    mesh  = mesh,
