...
```{r}
source(file = "./conf.R")
N <- 50 # Nombre minimal de biens par unité géographique
```
...
datXval <- kfold_compute(datXval, fn_estim = fn_estim_LM1, radix = "kfold_LM1", 
                         verbose = FALSE)
tab <- kfold_results(datXval, name_real = nm_var, name_esti = "kfold_LM1.S.estim", flag.fig = TRUE, stats = c("number", "min", "max", "mean", "MAE", "RMSE"))
...
datXval <- kfold_compute(datXval, fn_estim = fn_estim_LM2, radix = "kfold_LM2", 
                         verbose = FALSE)
tab <- kfold_results(datXval, name_real = nm_var, name_esti = "kfold_LM2.S.estim", flag.fig = TRUE, stats = c("number", "min", "max", "mean", "MAE", "RMSE"))
knitr::kable(round(tab,2), caption = paste0("Validation croisée par K-fold (K=", n.fold,")"))
